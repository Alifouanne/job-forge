...
`
      ...
      if (recentJobs.length > 0) {
        await step.run("send-email", async () => {
          ...
          $${job.salaryFrom.toLocaleString()} - $${job.salaryTo.toLocaleString()}</p>
          </div>
          `
            )
            .join("");
          await resend.emails.send({
            from: "JobForge <onboarding@resend.dev>",
            to: ["alifouanne7@gmail.com"],
    ...
    return { userId, message: "Competed 30 day job listing notifications" };
  }
);
